import React from 'react'
import Navbar from './Navbar'
import { Link } from 'react-router-dom'
import "../../App.css"
import { FaPlus } from "react-icons/fa6";
import { Table } from 'antd';
import { GoEye } from "react-icons/go";
import { IoIosPrint } from "react-icons/io";
import { RiDeleteBin6Line } from "react-icons/ri";
import { useState } from 'react';
// const columns = [
//     {
//       title: 'No',
//       width: 10,
//       dataIndex: 'No',
//       key: 'No',
     
//     },
//     {
//       title: 'Quotation No',
//       width: 10,
//       dataIndex: 'QuotationsNo',
//       key: 'QuotationsNo',
     
//     },
//     {
//       title: 'Date',
//       dataIndex: 'Date',
//       key: '1',
//       width: 10,
//     },
//     {
//       title: 'Customer Name',
//       dataIndex: 'Customername',
//       key: '2',
//       width: 10,
//     },
//     {
//       title: 'Total Amount',
//       dataIndex: 'Totalamount',
//       key: '3',
//       width: 10,
//     },
//     {
//       title: 'Status',
//       dataIndex: 'status',
//       key: '4',
//       width: 10,
//     },
    
//     {
//     title: 'Action',
//     dataIndex: 'Action',
//     key: '6',
//     width: 10,
//     render: () => (
//         <div className=' flex'>
//       <Link to={'/quotations/id'} className=" bg-white border-[1px] border-blue-500 text-[19px] p-2 text-blue-500  hover:bg-blue-500 hover:text-white "><GoEye /></Link>
      
//       </div>
//     ),
//   },
  
//   ];
  // const data = [];
  // for (let i = 0; i < 100; i++) {
  //   data.push({
  //     No: i,
  //     QuotationsNo: `Edward ${i}`,
  //     Customername: 32,
  //     Date: `London Park no. ${i}`,
  //     Payment:i*4,
  //     Totalamount:i+i,
  //     status:`sent`,
  //     Action:`del`
  //   });
  // }
const Quotations = () => {
  const data = [
    // Add your data here
    { col1: '0', col2: 'https://cdn.pixabay.com/photo/2024/05/08/17/45/animal-8748794_640.jpg',  col4: '1 month ago', col5: 'button',col6:"opop",col7:"ok" },
    { col1: '0', col2: 'https://cdn.pixabay.com/photo/2024/05/08/17/45/animal-8748794_640.jpg',  col4: '1 month ago', col5: 'button',col6:"opop",col7:"ok" },{ col1: '0', col2: 'https://cdn.pixabay.com/photo/2024/05/08/17/45/animal-8748794_640.jpg',  col4: '1 month ago', col5: 'button',col6:"opop",col7:"ok" },
    { col1: '0', col2: 'https://cdn.pixabay.com/photo/2024/05/08/17/45/animal-8748794_640.jpg',  col4: '1 month ago', col5: 'button',col6:"opop",col7:"ok" },
    { col1: '0', col2: 'https://cdn.pixabay.com/photo/2024/05/08/17/45/animal-8748794_640.jpg',  col4: '1 month ago', col5: 'button',col6:"opop",col7:"ok" },
    { col1: '0', col2: 'https://cdn.pixabay.com/photo/2024/05/08/17/45/animal-8748794_640.jpg',  col4: '1 month ago', col5: 'button',col6:"opop",col7:"ok" }, 
  ];

const [currentPage, setCurrentPage] = useState(1);
const itemsPerPage = 5;

// Calculate the total number of pages
const totalPages = Math.ceil(data.length / itemsPerPage);

// Get the current items to display based on the current page
const currentItems = data.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);

const handleNextPage = () => {
    setCurrentPage((prevPage) => Math.min(prevPage + 1, totalPages));
};

const handlePreviousPage = () => {
    setCurrentPage((prevPage) => Math.max(prevPage - 1, 1));
};


  return (
<>
<Navbar />

<div className='  w-full flex justify-center allorders bg-[#f6f8fb] mt-16 py-5 lg:mt-32'>
        <div className='w-10/12 border-[1px] rounded-md bg-white'>
        <div className=' flex justify-between p-5 border-b-[1px]'>
<div className=' font-sans font-medium text-gray-700 text-[18px]'>Quotations</div>
<Link to={'/quotations/create'}>
<div className=' cursor-pointer'>
<button className=' bg-green-500 border-[1px] text-white p-2 rounded-md hover:bg-white hover:text-green-500  hover:border-green-500'>
    <FaPlus className=' text-[25px] '/>
</button>
</div>
</Link>
        </div>

        <div className=' flex p-5 justify-end'>
            <div>
            <span>Search: </span>
         <input type='search' className=' border-[1px] border-gray-500 rounded-md p-1 ml-1'/>
         </div>
        </div>

        {/* <div className=' flex justify-between p-5 border-b-[1px]'>
        <Table
        className=' border-[1px]'
    columns={columns}
    dataSource={data}
    scroll={{
      x: 1500,
      y: 300,
    }}
  />

        </div> */}

<div className='flex justify-between border-b-[2px] mb-3 pb-3 '>
                        <div className="w-full flex justify-center px-4">
                            <div className="w-full h-[300px] overflow-auto mt-5 ">
                                <table className="w-full border-[2px] bg-white  border-gray-300">
                                    <thead>
                                        <tr className="bg-gray-300 border-[2px] text-blue-600 leading-10">
                                            <th className="px-4 py-2 border-[1px] border-gray-400 lg:w-[70px]">No</th>
                                            <th className="   px-4 py-2 border-[1px] border-gray-400 lg:w-[170px]">Quotation No</th>
                                            <th className="px-4 py-2 border-[1px] border-gray-400 lg:w-[120px]">Date</th>
                                            <th className=" px-4 py-2 border-[1px] border-gray-400 lg:w-[140px]">Customer Name</th>
                                            <th className="px-4 py-2 border-[1px] border-gray-400 lg:w-[150px]">Total amount</th>
                                            <th className="px-4 py-2 border-[1px] border-gray-400 lg:w-[120px]">Status</th>
                                            <th className="px-4 py-2 border-[1px] border-gray-400 lg:w-[80px]">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {currentItems.map((row, index) => (
                                            <tr key={index} className="border border-gray-300">
                                                <td className="px-4 py-2 border border-gray-300">{row.col1}</td>
                                                <td className="p-1 border border-gray-300"><img className=' h-[50px] w-full' src={row.col2} alt="" /></td>
                                                <td className="px-4 py-2 border border-gray-300">{row.col5}</td>
                                                <td className="px-4 py-2 border border-gray-300">{row.col6}</td>
                                                <td className="px-4 py-2 border border-gray-300">{row.col6}</td>
                                                <td className="px-4 py-2 border border-gray-300">{row.col6}</td>

                                                <td className="px-4 py-2 border border-gray-300 lg:px-0">
                                                    <div className='flex justify-around'>
                                                        <Link to={'/quotations/id'} className="bg-white border-[1px] mr-2 border-blue-500 text-[19px] p-2 text-blue-500 hover:bg-blue-500 hover:text-white"><GoEye /></Link>
                                                         </div>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
                                </table>
                            </div>
                        </div>
                    </div>

<div className=' w-full flex justify-center '>
                    <div className='flex w-11/12 justify-between py-5'>
                        <button onClick={handlePreviousPage} disabled={currentPage === 1} className="bg-blue-500 text-white px-4 py-2 rounded disabled:opacity-50">
                            Previous
                        </button>
                        <span>Page {currentPage} of {totalPages}</span>
                        <button onClick={handleNextPage} disabled={currentPage === totalPages} className="bg-blue-500 text-white px-4 py-2 rounded disabled:opacity-50">
                            Next
                        </button>
                    </div>
</div>  


       
        </div>
      </div>
    </>    

  )
}

export default Quotations;

